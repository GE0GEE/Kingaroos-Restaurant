...
export interface SiteContent {
  ...
  siteTexts: {
    ...
    welcomeText1: string;
    welcomeText2: string;
    welcomeImage1Caption: string; // <-- ADDED
    welcomeImage2Caption: string; // <-- ADDED
    ...
    homePhone: string;
    homeEmail: string;
    googleMapsUrl: string; // <-- ADDED for consistency
    ...
    hoursWeekday: string; // <-- RENAMED for consistency
    hoursWeekend: string; // <-- RENAMED for consistency
    hoursSunday: string; // <-- RENAMED for consistency
...
const defaultSiteContent: SiteContent = {
  ...
  heroImages: [
    ...
    { url: "/placeholder.svg", alt: "Delicious food spread", gradient: "from-aussie-orange/70 to-brown-400/80" },
    { url: "/placeholder.svg", alt: "Happy rescue dogs", gradient: "from-aussie-eucalyptus/60 to-sand-400/80" },
  ],
  welcomeImages: [ // <-- ADDED
    { url: "/placeholder.svg", alt: "Happy customers with dogs" },
    { url: "/placeholder.svg", alt: "Fresh, delicious Australian-inspired cuisine" },
  ],
  aboutImages: {
  ...
  siteTexts: {
    ...
    welcomeImage1Caption: "Smiling customers enjoying our dog-friendly atmosphere", // <-- ADDED
    welcomeImage2Caption: "Fresh, delicious Australian-inspired cuisine", // <-- ADDED
    ...
    googleMapsUrl: "https://www.google.com/maps/embed?pb=!1m14!1m8!1m3!1d15751.867549789918!2d123.29718759559996!3d9.247256723398154!3m2!1i1024!2i768!4f13.1!3m3!1m2!1s0x33ab69f7f93062cf%3A0xedaf9d009a9047d0!2sKingaroo's%20Seaview%20Resto%20Bar!5e0!3m2!1sen!2sus!4v1749525539183!5m2!1sen!2sus", // <-- ADDED
    ...
    hoursWeekday: "11am - 9pm", // <-- RENAMED
    hoursWeekend: "11am - 10pm", // <-- RENAMED
    hoursSunday: "10am - 8pm", // <-- RENAMED
...
export function AdminProvider({ children }: { children: ReactNode }) {
    ...
    try {
      ...
      if (response.ok) {
        ...
        const mergedContent = {
          ...
          welcomeImages: content.welcomeImages || defaultSiteContent.welcomeImages,
        };
      ...
      console.warn("⚠️ Server not available, using localStorage fallback:", error);
      ...
      if (saved) {
        try {
          ...
          const mergedContent = {
            ...
            welcomeImages: parsedContent.welcomeImages || defaultSiteContent.welcomeImages,
          };
          setSiteContent(mergedContent);
  ...
  const updateSiteContent = async (updates: Partial<SiteContent>) => {
    ...
    if (isServerConnected) {
      try {
        const updatedContent = await apiCall("/content", { method: "PUT", body: JSON.stringify(updates) });
  ...
  const addDog = async (dog: Omit<Dog, "id">) => {
    ...
    if (isServerConnected) {
      try {
        await apiCall("/dogs", { method: "POST", body: JSON.stringify(dog) });
    ...
    const newContent = { ...siteContent, dogs: [...siteContent.dogs, newDog] };
    setSiteContent(newContent);
    saveToLocalStorage(newContent);
  };
  ...
  const updateDog = async (id: string, updates: Partial<Dog>) => {
    if (isServerConnected) {
      try {
        await apiCall(`/dogs/${id}`, { method: "PUT", body: JSON.stringify(updates) });
        await initializeContent();
        return;
      } catch (error) {
    ...
    const newContent = { ...siteContent, dogs: siteContent.dogs.map((dog) => (dog.id === id ? { ...dog, ...updates } : dog)) };
    setSiteContent(newContent);
    saveToLocalStorage(newContent);
  };
  ...
  const deleteDog = async (id: string) => {
    if (isServerConnected) {
      try {
        await apiCall(`/dogs/${id}`, { method: "DELETE" });
        await initializeContent();
        return;
      } catch (error) {
    ...
    const newContent = { ...siteContent, dogs: siteContent.dogs.filter((dog) => dog.id !== id) };
    setSiteContent(newContent);
    saveToLocalStorage(newContent);
  };
  ...
  const addMenuItem = async (item: Omit<MenuItem, "id">) => {
    ...
    if (isServerConnected) {
      try {
        await apiCall("/menu-items", { method: "POST", body: JSON.stringify(item) });
        await initializeContent();
        return;
      } catch (error) {
        console.error("Server add menu item failed, using localStorage:", error);
        setIsServerConnected(false);
      }
    }
    const newContent = { ...siteContent, menuItems: [...siteContent.menuItems, newItem] };
    setSiteContent(newContent);
    saveToLocalStorage(newContent);
  };
  ...
  const updateMenuItem = async (id: string, updates: Partial<MenuItem>) => {
    if (isServerConnected) {
      try {
        await apiCall(`/menu-items/${id}`, { method: "PUT", body: JSON.stringify(updates) });
        await initializeContent();
        return;
      } catch (error) {
        console.error("Server update menu item failed, using localStorage:", error);
        setIsServerConnected(false);
      }
    }
    const newContent = { ...siteContent, menuItems: siteContent.menuItems.map((item) => (item.id === id ? { ...item, ...updates } : item)) };
    setSiteContent(newContent);
    saveToLocalStorage(newContent);
  };
  ...
  const deleteMenuItem = async (id: string) => {
    if (isServerConnected) {
      try {
        ...
        console.error("Server delete menu item failed, using localStorage:", error);
        setIsServerConnected(false);
      }
    }
    const newContent = { ...siteContent, menuItems: siteContent.menuItems.filter((item) => item.id !== id) };
    setSiteContent(newContent);
    saveToLocalStorage(newContent);
  };
  ...
  const addEvent = async (event: Omit<Event, "id">) => {
    ...
    const newContent = { ...siteContent, events: [...siteContent.events, newEvent] };
    setSiteContent(newContent);
    saveToLocalStorage(newContent);
  };
  ...
  const updateEvent = async (id: string, updates: Partial<Event>) => {
    if (isServerConnected) {
      try {
        await apiCall(`/events/${id}`, { method: "PUT", body: JSON.stringify(updates) });
        await initializeContent();
        return;
      } catch (error) {
    ...
    const newContent = { ...siteContent, events: siteContent.events.map((event) => (event.id === id ? { ...event, ...updates } : event)) };
    setSiteContent(newContent);
    saveToLocalStorage(newContent);
  };
  ...
  const deleteEvent = async (id: string) => {
    ...
    const newContent = { ...siteContent, events: siteContent.events.filter((event) => event.id !== id) };
    setSiteContent(newContent);
    saveToLocalStorage(newContent);
  };
  ...
  const addPromotion = async (promo: Omit<Promotion, "id">) => {
    ...
    if (isServerConnected) {
      try {
        await apiCall("/promotions", { method: "POST", body: JSON.stringify(promo) });
        await initializeContent();
        return;
      } catch (error) {
        console.error("Server add promotion failed, using localStorage:", error);
        setIsServerConnected(false);
      }
    }
    const newContent = { ...siteContent, promotions: [...siteContent.promotions, newPromo] };
    setSiteContent(newContent);
    saveToLocalStorage(newContent);
  };
  ...
  const updatePromotion = async (id: string, updates: Partial<Promotion>) => {
    if (isServerConnected) {
      try {
        await apiCall(`/promotions/${id}`, { method: "PUT", body: JSON.stringify(updates) });
        await initializeContent();
        return;
      } catch (error) {
        console.error("Server update promotion failed, using localStorage:", error);
        setIsServerConnected(false);
      }
    }
    const newContent = { ...siteContent, promotions: siteContent.promotions.map((promo) => (promo.id === id ? { ...promo, ...updates } : promo)) };
    setSiteContent(newContent);
    saveToLocalStorage(newContent);
  };
  ...
  const deletePromotion = async (id: string) => {
    if (isServerConnected) {
      try {
        ...
        console.error("Server delete promotion failed, using localStorage:", error);
        setIsServerConnected(false);
      }
    }
    const newContent = { ...siteContent, promotions: siteContent.promotions.filter((promo) => promo.id !== id) };
    setSiteContent(newContent);
    saveToLocalStorage(newContent);
  };

  return (
    <AdminContext.Provider value={{ isLoggedIn, login, logout, siteContent, loading, isServerConnected, updateSiteContent, addDog, updateDog, deleteDog, addMenuItem, updateMenuItem, deleteMenuItem, addEvent, updateEvent, deleteEvent, addPromotion, updatePromotion, deletePromotion }}>
      {children}

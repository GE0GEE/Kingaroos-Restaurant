    ...
    <Layout>
      ...
      <div className="relative bg-stone-900 text-white py-20 px-4 text-center overflow-hidden">
        ...
        <h1 className="relative font-heading text-4xl md:text-6xl font-extrabold tracking-tight mb-3">
          {siteContent.siteTexts.dogRescueTitle || "Dog Rescue"}
        </h1>
        <p className="relative font-body text-stone-400 text-base md:text-lg max-w-md mx-auto leading-relaxed">
          {siteContent.siteTexts.dogRescueSubtitle || "Every meal you enjoy helps a rescue dog find their forever home."}
        </p>
      </div>
      ...
      <section className="py-16 px-4">
        <div className="max-w-7xl mx-auto">
          <h2 className="font-heading text-4xl font-bold text-center text-brown-800 mb-4">
          ...
          {siteContent.dogs.length === 0 ? (
            <div className="text-center py-12">
              ...
              <p className="font-body text-brown-600">No rescue dogs to display at the moment. Check back soon!</p>
            </div>
          ) : (
                  ...
                  <CardContent className="p-0">
                    <div className="relative">
                      <div className="w-full h-64 bg-sand-200 rounded-t-lg overflow-hidden">
                        {dog.afterImage && dog.afterImage !== "/placeholder.svg" ? (
                          <img src={dog.afterImage} alt={`${dog.name} after recovery`} className="w-full h-full object-cover" />
                        ) : (
                          <div className="w-full h-full flex items-center justify-center">
                            <div className="text-center space-y-2">
                              <Heart className="h-12 w-12 text-aussie-orange mx-auto" />
                              <p className="font-body text-brown-600 text-sm">Click to see {dog.name}'s story</p>
                            </div>
                          </div>
                        )}
                    ...
                    <div className="p-6 space-y-4">
                      <div className="flex items-center justify-between">
                        <h3 className="font-heading text-2xl font-bold text-brown-800">{dog.name}</h3>
                        <span className="font-body text-brown-600 text-sm">{dog.age}</span>
                      </div>
                      <p className="font-body text-brown-600 font-semibold">{dog.breed}</p>
                      <p className="font-body text-brown-600 leading-relaxed">{dog.personality}</p>
                      <div className="pt-2">
                        <Button variant="outline" size="sm" className="w-full border-aussie-orange text-aussie-orange hover:bg-aussie-orange hover:text-white">
          ...
          {selectedDog && (
            <div className="space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
                <div>
                  <p className="font-body text-brown-600"><span className="font-semibold">Breed:</span> {selectedDog.breed}</p>
                  <p className="font-body text-brown-600"><span className="font-semibold">Age:</span> {selectedDog.age}</p>
                </div>
                <div>
                  <p className="font-body text-brown-600"><span className="font-semibold">Personality:</span> {selectedDog.personality}</p>
                </div>
              </div>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div className="space-y-4">
                  <h3 className="font-heading text-xl font-bold text-brown-800 text-center">Before Rescue</h3>
                  <Card className="border-sand-200">
                    <CardContent className="p-4">
                      <div className="w-full h-64 bg-sand-200 rounded-lg flex items-center justify-center mb-4">
                        {selectedDog.beforeImage && selectedDog.beforeImage !== "/placeholder.svg" ? (
                          <img src={selectedDog.beforeImage} alt={`${selectedDog.name} before rescue`} className="w-full h-full object-cover rounded-lg" />
                        ) : (
                          <div className="text-center space-y-2">
                            <Heart className="h-12 w-12 text-brown-400 mx-auto" />
                            <p className="font-body text-brown-500 text-sm">Before: {selectedDog.name} when first rescued</p>
                          </div>
                        )}
                      </div>
                      <p className="font-body text-brown-600 text-center text-sm italic">When we first found {selectedDog.name}</p>
                    </CardContent>
                  </Card>
                </div>
                <div className="space-y-4">
                  <h3 className="font-heading text-xl font-bold text-brown-800 text-center">After Recovery</h3>
                  <Card className="border-sand-200">
                    <CardContent className="p-4">
                      <div className="w-full h-64 bg-sand-200 rounded-lg flex items-center justify-center mb-4">
                        {selectedDog.afterImage && selectedDog.afterImage !== "/placeholder.svg" ? (
                          <img src={selectedDog.afterImage} alt={`${selectedDog.name} after recovery`} className="w-full h-full object-cover rounded-lg" />
                        ) : (
                          <div className="text-center space-y-2">
                            <Heart className="h-12 w-12 text-aussie-orange mx-auto" />
                            <p className="font-body text-aussie-orange text-sm">After: {selectedDog.name} today</p>
                          </div>
                        )}
                      </div>
                      <p className="font-body text-brown-600 text-center text-sm italic">{selectedDog.name} today - happy and healthy!</p>
                    </CardContent>
                  </Card>
                </div>
              </div>
              <Card className="bg-cream-50 border-sand-200">
                <CardContent className="p-6">
                  ...
                  <p className="font-body text-brown-600 leading-relaxed">{selectedDog.rescueStory}</p>
                </CardContent>
              </Card>
            </div>
          )}
            ...
            <Card className="bg-cream-50 border-sand-200 max-w-2xl mx-auto">
              <CardContent className="p-6">
                <div className="flex items-center justify-center space-x-3 mb-4">
                  ...
                  <h3 className="font-heading text-lg font-bold text-brown-800">Get Involved</h3>
                </div>

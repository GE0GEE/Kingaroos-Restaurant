...
function DogForm({ dog, onSubmit }: { dog: Partial<Dog>; onSubmit: (dog: Partial<Dog>) => void; }) {
  ...
  const handleImageUpload = (field: "beforeImage" | "afterImage", e: React.ChangeEvent<HTMLInputElement>) => {
    const file = e.target.files?.[0];
    if (file) {
      const reader = new FileReader();
      reader.onload = (e) => setFormData((prev) => ({ ...prev, [field]: e.target?.result as string }));
  ...
  return (
    <div className="space-y-4">
      ...
      <div><Label>Age</Label><Input value={formData.age || ""} onChange={(e) => setFormData((prev) => ({ ...prev, age: e.target.value }))} /></div>
      ...
      <div className="grid grid-cols-2 gap-4">
        <div>
          <Label>Before Image</Label><Input type="file" accept="image/*" onChange={(e) => handleImageUpload("beforeImage", e)} />
          {formData.beforeImage && <img src={formData.beforeImage} alt="Before" className="w-full h-32 object-cover rounded-md mt-2" />}
        </div>
        <div>
          <Label>After Image</Label><Input type="file" accept="image/*" onChange={(e) => handleImageUpload("afterImage", e)} />
...
function MenuItemForm({ item, onSubmit }: { item: Partial<MenuItem>; onSubmit: (item: Partial<MenuItem>) => void; }) {
  ...
  return (
    <div className="space-y-4">
      ...
      <div><Label>Item Image</Label><Input type="file" accept="image/*" onChange={handleImageUpload} />{formData.image && <img src={formData.image} alt="Menu item" className="w-full h-32 object-cover rounded-md mt-2" />}</div>
...
export default function Admin() {
  ...
  const handleSaveTexts = async () => {
    try { await updateSiteContent({ siteTexts: editingTexts }); alert("Site texts updated!"); }
  ...
  const handleGenericImageUpload = (updateFunction: (dataUrl: string) => Promise<void>) => async (e: React.ChangeEvent<HTMLInputElement>) => {
    const file = e.target.files?.[0];
    if (!file) return;
    const reader = new FileReader();
    reader.onload = async (event) => {
      try {
        await updateFunction(event.target?.result as string);
        alert("Image updated!");
      } catch (error) {
  ...
  const handleAboutFormImageUpload = (field: keyof typeof editingImages) => (e: React.ChangeEvent<HTMLInputElement>) => {
    const file = e.target.files?.[0];
    if (file) {
      const reader = new FileReader();
      reader.onload = (event) => setEditingImages((prev) => ({ ...prev, [field]: event.target?.result as string }));
      reader.readAsDataURL(file);
    }
  };
            ...
            <TabsContent value="images">
              <div className="space-y-6">
                <Card>
                  ...
                  <CardContent><Input type="file" accept="image/*" onChange={handleGenericImageUpload((result) => updateSiteContent({ logoImage: result }))} />{siteContent.logoImage && <img src={siteContent.logoImage} alt="Logo" className="w-32 h-32 mt-4" />}</CardContent>
                </Card>
                <Card>
                  <CardHeader><CardTitle>Hero Images</CardTitle></CardHeader>
                  <CardContent className="grid md:grid-cols-3 gap-4">
                    {(siteContent.heroImages ?? []).map((image, index) => (
                      <div key={index}><Label>Hero Image {index + 1}</Label><Input type="file" accept="image/*" onChange={handleGenericImageUpload((result) => {const newImages = [...siteContent.heroImages]; newImages[index] = {...newImages[index], url: result}; return updateSiteContent({ heroImages: newImages });})}/>{image.url && <img src={image.url} alt={image.alt} className="w-full h-32 mt-2 object-cover" />}</div>
                    ))}
                  </CardContent>
                </Card>
                <Card>
                  ...
                  <CardContent className="grid md:grid-cols-2 gap-4">
                    {(siteContent.welcomeImages ?? []).map((image, index) => (
                      <div key={index}><Label>Welcome Image {index + 1}</Label><Input type="file" accept="image/*" onChange={handleGenericImageUpload((result) => {const newImages = [...siteContent.welcomeImages]; newImages[index] = {...newImages[index], url: result}; return updateSiteContent({ welcomeImages: newImages });})}/>{image.url && <img src={image.url} alt={image.alt} className="w-full h-32 mt-2 object-cover" />}</div>
                    ))}
                  </CardContent>
                </Card>
                <Card>
